...
class AttnProcessor2_1:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #     query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        # )
        hidden_states, attention_weight = scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, attention_map=True
        )
        ...
        if attention_weight.shape[0] == 4 and attn.t <= 500 and False:
            print(attn.t)
            global COUNT
            for k, object_index in enumerate(object_indexes):
                visualize_grid_to_grid(attention_weight[3, 0, :, object_index], 'attn_app_text|{}|_{}_{}'.format(COUNT, attn.t, k), alpha=0.6)
            COUNT += 1
