...
class CtrlXMultiplePipeline(StableDiffusionXLPipeline):  # diffusers==0.28.0
    ...
    def __call__(
            ...
            appearance_images: Optional[PipelineImageInput] = None,
        ...
        if True:
            clean_appearance_latents_list = []
            for i, appearance_image in enumerate(appearance_images):
                appearance_images[i] = preprocess(  # Center crop + resize
                ...
                _, clean_appearance_latents = self.prepare_latents(
                    appearance_images[i], batch_size, num_images_per_prompt, num_channels_latents, height, width,
                    prompt_embeds.dtype, device, generator, appearance_latents,
                )
                clean_appearance_latents_list.append(clean_appearance_latents)
        else:
            clean_appearance_latents_list = None
        appearance_latents = latents if appearance_latents is None else appearance_latents
        # print(appearance_latents.shape)
        list_app_latents = []
        for i in range(len(appearance_images)):
            list_app_latents.append(appearance_latents)
        # appearance_latents = torch.cat(list_app_latents, dim=0)
        # appearance_latents = appearance_latents.repeat(len(appearance_images), 1, 1, 1)
        # print(appearance_latents.shape)
            ...
            for i, t in enumerate(timesteps):
                ...
                if i == appearance_control_stop_i:
                    if "appearance_uncond" not in batch_order:
                        for j in range(len(appearance_images)):
                            batch_order.remove(f"{j}")
                #         batch_order.remove("appearance_cond")
                ...
                appearance_latent_model_input_list = []
                for i in range(len(appearance_images)):
                    appearance_latent_model_input_list.append(
                        self.scheduler.scale_model_input(list_app_latents[i], t)
                    )
                ...
                for i in range(len(appearance_images)):
                    all_latent_model_input[f"{i}"] = appearance_latent_model_input_list[i][0:1]
                all_prompt_embeds = {
                ...
                for i in range(len(appearance_images)):
                    all_prompt_embeds[f"{i}"] = appearance_prompt_embeds[1:2]
                all_add_text_embeds = {
                ...
                for i in range(len(appearance_images)):
                    all_add_text_embeds[f"{i}"] = appearance_add_text_embeds[1:2]
                all_time_ids = {
                ...
                for i in range(len(appearance_images)):
                    all_time_ids[f"{i}"] = add_time_ids[1:2]
                ...
                appearance_noise_pred_list = []
                for i in range(len(appearance_images)):
                    key = f"{i}"
                    appearance_noise_pred = all_noise_pred[key] \
                        if key in batch_order else noise_pred
                    appearance_noise_pred_list.append(appearance_noise_pred)
                ...
                list_noise_pred = [structure_noise_pred] + appearance_noise_pred_list + [noise_pred]
                # concat_noise_pred = torch.cat(
                #     [structure_noise_pred, appearance_noise_pred, noise_pred], dim=0,
                # )
                concat_noise_pred = torch.cat(list_noise_pred, dim=0)
                list_latents = [structure_latents] + list_app_latents + [latents]
                concat_latents = torch.cat(list_latents, dim=0)
                ...
                # print(structure_latents.shape)
                # print(latents.shape)
                # print('heerre')
                structure_latents = new_latents[:structure_latents.shape[0]]
                for i in range(len(list_app_latents)):
                    list_app_latents[i] = new_latents[i+1:i+2]
                latents = new_latents[-latents.shape[0]:]
                ...
                for i in range(len(list_app_latents)):
                    if clean_appearance_latents_list[i] is not None:
                        list_app_latents[i] = noise_prev(self.scheduler, t, clean_appearance_latents_list[i])
